...
from mosaicfm.data import CountDataset, DataCollator, GeneSeqCollator
from mosaicfm.model import SCGPTModel
from mosaicfm.tokenizer import GeneVocab


def get_batch_embeddings_mfmv2(
    adata: AnnData,
    model: SCGPTModel,
    vocab: GeneVocab,
...
def get_batch_embeddings(
    ...
    with torch.no_grad(), torch.amp.autocast(
        ...
        pbar = tqdm(total=len(dataset), desc="Embedding cells")
        for data_dict in data_loader:
            input_gene_ids = data_dict["in_gene_ids"].to(device)
            input_exprs = data_dict["in_exprs"].to(device)

            # encode
            token_gene_embs = model.gene_encoder(
                input_gene_ids,
            )  # (batch, in_seq_len, embsize)
            token_values = model.expression_encoder(
                input_exprs,
            )  # (batch, in_seq_len, embsize)
            total_embs = token_gene_embs + token_values  # (batch, in_seq_len, embsize)

            if collator_cfg.get("use_chem_token", False):
                drug_ids = data_dict["drug_ids"].to(device)  # (batch, in_seq_len)
                drug_embs = model.chem_encoder(drug_ids)  # (batch, embsize)
                total_embs[:, 1, :] = drug_embs  # (batch, in_seq_len, embsize)

            embeddings = model.transformer_encoder(
