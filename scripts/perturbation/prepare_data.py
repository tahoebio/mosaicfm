...
def prepare_data(data_path, dataset_name, vocab_path, batch_size=1):
    ...
    with open(vocab_path,"r") as f:
        ...
        gene_list = pert_data.adata.var["gene_name"].tolist() #n_genes in adamson: 5060
        mapped_gene_ids = [gene_to_id.get(gene_name, gene_to_id["<pad>"]) for 
                        gene_name in gene_list]

        percent_mapped = (len(set(gene_list) & set(gene_to_id.keys())) /  len(set(gene_list)))*100
    ...
    np.savez(os.path.join(data_dir, "mean_ctrl_log1p.npz"), gene_names = list(adata.var.gene_name), mean_ctrl = mean_ctrl)
    ...
    for split in splits:
        ...
        save_to_torch_dataset(data_loader, data_dir, dataset_name, split, mapped_gene_ids, gene_list)
...
def save_to_torch_dataset(data_loader, data_dir, dataset_name, split, mapped_gene_ids, gene_list):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vocab_path", required=True, help="path to model's vocab.json.")
    parser.add_argument("--batch_size", type=int, default=1, help="Batch size for processing data")
    """
    DATA_PATH = "/vevo/datasets/perturbation_datasets/"
    DATASET_NAME = "adamson"
    VOCAB_APTH = "/vevo/scgpt/checkpoints/release/scgpt-70m-1024-fix-norm-apr24-data/vocab.json"
    """
