#!/usr/bin/env python3
"""
Preprocess UniProt TSV file to extract relevant protein information for ESM-C embedding generation.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Preprocess UniProt data for protein embedding generation")
...
def main():
    ...
    df.columns = [col.replace(" ", "_").replace("(", "").replace(")", "").replace("-", "_").lower() 
    ...
    print(f"Sample of gene names: {', '.join(df['gene_names_primary'].dropna().sample(5).tolist())}")
