...
with h5py.File(embedding_file, "r") as f:
    ...
    gene_names_esmc = [name.decode('utf-8') if name else "" for name in f["gene_names"][:]]
    
    print(f"Loaded {len(esmc_embeddings)} ESM-C embeddings with dimension {esmc_embeddings.shape[1]}")
...
all_mappings = pd.merge(
    vocab_df,
    merged_with_esmc[['ensembl_id_short', 'esmc_idx', 'gene_names_primary']].drop_duplicates(),
    left_on='token',
...
print(f"Original embedding stats - Mean: {orig_mean.mean().item():.4f}, Std: {orig_std.mean().item():.4f}, Norm: {orig_norm.item():.4f}")
...
print(f"ESM-C embedding stats - Mean: {np.mean(esmc_mean):.4f}, Std: {np.mean(esmc_std):.4f}, Norm: {esmc_norm:.4f}")
...
for _, row in tqdm(all_mappings.iterrows(), desc="Replacing embeddings"):
    ...
    normalized_emb = (esmc_emb - esmc_mean) / (esmc_std + 1e-8)  # Avoid division by zero
    normalized_emb = normalized_emb * orig_std.detach().cpu().numpy() + orig_mean.detach().cpu().numpy()
    
    # Replace embedding
    new_embedding[vocab_idx] = torch.tensor(normalized_emb, dtype=original_embedding.dtype)
...
print(f"New embedding stats - Mean: {new_mean.mean().item():.4f}, Std: {new_std.mean().item():.4f}, Norm: {new_norm.item():.4f}")
...
print(f"Successfully replaced {len(all_mappings)} out of {len(vocab_df)} embeddings ({len(all_mappings)/len(vocab_df)*100:.2f}%)")
...
if not sample_row.empty:
    ...
    if not mapping_row.empty:
        ...
        if 'ensembl_id_short' in mapping_row.columns and mapping_row.iloc[0]['ensembl_id_short'] == sample_token:
            print(f"  Matched as Ensembl ID")
        elif 'gene_names_primary' in mapping_row.columns and mapping_row.iloc[0]['gene_names_primary'] == sample_token:
            print(f"  Matched as gene name")
        ...
        print(f"This token was not replaced with an ESM-C embedding")
else:
