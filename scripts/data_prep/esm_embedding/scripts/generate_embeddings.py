...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate protein embeddings using ESM-C",
    )
    parser.add_argument(
        "--input",
        type=str,
...
def main():
    ...
    try:
        ...
        print(
            "Error: Could not import ESM modules. Make sure the ESM package is installed.",
        )
    ...
    def get_embeddings(sequence):
        try:
            with torch.no_grad():  # Disable gradient calculation
                ...
                mean_pooled = (
                    logits_output.embeddings.mean(dim=1).squeeze(0).cpu().numpy()
                )
        ...
        with h5py.File(args.output, "r") as f:
            ...
            if start_idx == 0 or embedding_dim == 0:
                ...
                if sample_embedding is None:
                    print(
                        "Error: Failed to get sample embeddings. Check if the model is loaded correctly.",
                    )
                    return
    ...
    with h5py.File(args.output, mode) as f:
        ...
        if mode == "w" or "embeddings" not in f:
            ...
            f.create_dataset(
                "gene_names",
                data=df["gene_names_primary"].values.astype("S"),
            )
            if "ensembl_geneid" in df.columns:
                f.create_dataset(
                    "ensembl_ids",
                    data=df["ensembl_geneid"].values.astype("S"),
                )
            f.create_dataset(
                "sequence_lengths",
                data=df["length"].values.astype(np.int32),
            )
            ...
            f.attrs["flash_attention"] = (
                "enabled" if args.use_flash_attn else "disabled"
            )
    ...
    if failed_count > 0:
        print(
            f"Warning: Failed to process {failed_count} proteins ({failed_count/len(df):.2%} of total)",
        )
