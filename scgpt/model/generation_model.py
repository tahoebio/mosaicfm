...
class TransformerGenerator(nn.Module):
    ...
    def forward(
        ...
        mlm_output = self.decoder(
            transformer_output, values, input_pert_flags
        ...
        if CLS:
            output["cls_output"] = self.cls_decoder(transformer_output, input_pert_flags)  # (batch, n_cls)
            mask = (torch.sigmoid(output["cls_output"]) > 0.5).float().squeeze(-1)
            output["mlm_output"] = output["mlm_output"] * mask
...
class AffineExprDecoder(nn.Module):
    ...
    def forward(self, x: Tensor, values: Tensor, input_pert_flags: Tensor) -> Tensor:
        """
        Args:
        ...
        """
        # Use batch labels
        indices = torch.nonzero(input_pert_flags, as_tuple=True)
        input_flag_embedding = x[indices].clone()
        input_flag_embedding = input_flag_embedding.unsqueeze(1).expand(-1, x.size(1), -1)
        #x = torch.cat([x, input_flag_embedding], dim=-1)
        # TODO
        x = x + input_flag_embedding
...
class ClsDecoder(nn.Module):
    ...
    def forward(self, x: Tensor, input_pert_flags: Tensor) -> Tensor:
        """
        Args:
